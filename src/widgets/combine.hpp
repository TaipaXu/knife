...
namespace Ui
{
    ...
    private slots:
        void handleOutputNameChanged(const QString &text) const;
        void handleOutputDirButtonClicked();
        void start();
        void handleCombineSuccess();

    private:
        void handleCombineFail(const QString &message);

    private:
        QLineEdit *outputNameInput;
        QLineEdit *outputDirInput;
        QPushButton *startButton;
        Loading *loading;
        QModel::Combine *controlModel;
        Tool::Pdf *pdf;
        bool working;
    };
} // namespace Ui
